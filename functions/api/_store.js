// ===========================================================
//  «Датка» — бэкенддин жалпы жардамчы модулу (Cloudflare KV)
//  - демейки маалыматтар (defaults)
//  - KV коллекциялары менен иштөө (окуу/жазуу)
//  - администратордун аутентификациясы
//  "_" менен башталган файл өзүнчө маршрут болуп саналбайт.
// ===========================================================

export const DEFAULT_ADMIN_PASSWORD = "datka2026";
...
export const DEFAULTS = {
  ...
  settings: {
    salonName: "Датка",
    tagline: "сулуулук салону",
    heroEyebrow: "2018-жылдан бери Ош шаарында",
    heroTitle: "Сулуулук — бул өзүңө болгон сүйүү",
    heroSubtitle: "«Датка» салонунда тажрыйбалуу шеберлер, заманбап жабдуулар жана жагымдуу атмосфера сизди күтүүдө.",
    stat1Value: "7+",  stat1Label: "жылдык тажрыйба",
    stat2Value: "12",  stat2Label: "кесипкөй шебер",
    stat3Value: "9000+", stat3Label: "ыраазы кардар",
    phone1: "+996 700 123 456",
    phone2: "+996 555 123 456",
    address: "Кыргызстан, Ош шаары, Курманжан Датка көчөсү, 123",
    hours: "Дүйшөмбү – Ишемби: 09:00 – 20:00; Жекшемби: 10:00 – 18:00",
    instagram: "#",
    whatsapp: "#",
    telegram: "#",
  },
...
export const COLLECTIONS = ["services", "masters", "reviews", "gallery", "settings"];
...
// KV коллекциясын окуу (бош болсо — демейки маани кайтарат)
export async function readCol(env, name) {
  if (!env.DATKA_BOOKINGS) return DEFAULTS[name];
  const raw = await env.DATKA_BOOKINGS.get(`col:${name}`);
  if (raw == null) return DEFAULTS[name];
  try { return JSON.parse(raw); } catch { return DEFAULTS[name]; }
}

export async function writeCol(env, name, value) {
  await env.DATKA_BOOKINGS.put(`col:${name}`, JSON.stringify(value));
}

// ---------- Администратордун аутентификациясы ----------
export async function getAdminPassword(env) {
  if (!env.DATKA_BOOKINGS) return DEFAULT_ADMIN_PASSWORD;
  const p = await env.DATKA_BOOKINGS.get("admin:password");
...
export async function createSession(env) {
  ...
  await env.DATKA_BOOKINGS.put(`session:${token}`, "1", { expirationTtl: 60 * 60 * 24 });
  return token;
}

export async function isAuthed(request, env) {
  const token = request.headers.get("X-Admin-Token");
  if (!token || !env.DATKA_BOOKINGS) return false;
  const v = await env.DATKA_BOOKINGS.get(`session:${token}`);
  return v === "1";
}

export async function requireAuth(request, env) {
  if (await isAuthed(request, env)) return null;
  return json({ error: "Авторизация талап кылынат." }, 401);
}
